...
int simple_read(const char *path, char *buf, size_t size, off_t offset,
	...
	getMovieInfo(path, completeOutput);
	//completeOutput = "testestestestsetsetsetsetsetsetsetsetsetsetsetsetsetsetsetsetsetsetsetsetset";

 	if (count > 0 && completeOutput.length() > 0) {
		int i;
		for(i = 0; i < fmin(count, completeOutput.length()); i++) {
			buf[i] = (char)completeOutput.at(i);
		}
	}
...
int simple_write(const char *path, const char *buf, size_t size, off_t offset,
	...
	addComment(std::string(path), std::string(buf));
	return 1;
}
